...
const cart = () => {
    ...
    document.querySelector('.checkOut').addEventListener('click', function() {
        ...
        if (productos && productos.length > 0) {
            // Almacenar los productos del carrito en el almacenamiento local para su posterior recuperación en la página de compra
            localStorage.setItem('productosCompra', JSON.stringify(productos));
            
            // Redireccionar a la página de compra
            window.location.href = 'Pago.html';
        } else {
            // Mostrar un mensaje de alerta si no hay productos en el carrito
